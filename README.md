...
- main()
  Cette fonctionn permet dans un premier temps de récupérer les input de l'utilisateurs grace à argparse. Elle détermine
  ensuite si l'utilisateur veut utiliser la fonction pour chiffrer ou pour déchiffrer. Puis elle retourne à
  l'utilisateur le résultat des fonctions.
...
Cette fonction prend en entrée une phrase et une clé et retourne le codage_vigenere du texte en utilisant le chiffrement
de Vigenère.
...
Cette fonction prend en paramètres une phrase chiffrée et une clé, et retourne la phrase déchiffrée en utilisant le
...
On ajoute les arguments "phrase" et "key" et également le paramètre nargs='?' pour préciser que l'argument peut-être
égal à zéro ou à 1. Autrement dit que l'argument peut être vide.
...
Ajout d'un argument 'flag' pour le decodage, il permet de préciser si l'utilisateur shouaite decoder un message plutôt
que de le decoder.
